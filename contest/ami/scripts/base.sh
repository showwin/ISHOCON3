...
# Install MySQL
sudo debconf-set-selections <<< 'mysql-server mysql-server/root_password password ishocon'
sudo debconf-set-selections <<< 'mysql-server mysql-server/root_password_again password ishocon'
sudo debconf-set-selections <<< 'mysql-service mysql-server/mysql-apt-config string 4'
sudo apt-get install -y mysql-server

# Install Ruby
export RUBY_VERSION=3.3.10
