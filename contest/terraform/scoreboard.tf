...
locals {
  main_js_content = replace(file("${path.module}/../scoreboard/dist/main.js"), "<<API_GATEWAY_DOMAIN_URL>>", aws_apigatewayv2_stage.scoreboard.invoke_url)
}

resource "aws_s3_object" "scoreboard_main_js" {
  bucket       = aws_s3_bucket.scoreboard.bucket
  key          = "dist/main.js"
  content      = local.main_js_content
  content_type = "application/javascript"
  etag         = md5(local.main_js_content)
