...
- [ ] (任意) variables.tf変更したいパラメーターを terraform.tfvars に定義
...
scoreboard_url = "http://ishocon3-scoreboard123456789.s3-website-ap-northeast-1.amazonaws.com"
```
...
```
$ task close-scoreboard  # 競技1時間前などにスコアを非表示にする
$ task reopen-scoreboard  # 非表示にしたスコアボードを再度表示する

$ task freeze-scoreboard  # 競技終了後にベンチマーカーを動かしてもスコアボードに反映されないようにする
$ task unfreeze-scoreboard  # スコアボードの凍結を解除し、ベンチマーカーの結果がスコアボードに反映されるようにする
...
- デフォルトでは、ISHOCON3の推奨インスタンスタイプである `c7i.xlarge` でインスタンスが起動します
