...
そこから4時間休憩し、B駅を9:40以降に出発するチケットの予約を試みます。このときスケジュール一覧で取得した列車一覧にB駅を9:40以降に出発するものが含まれていない場合はそれ以降の予約を停止します。つまり、A-C、C-Bの2区間分のチケットしか購入しません。

このように旅程に対して各区間でもっとも早い列車を選択、予約していきます。最も早い列車が満席で売り切れている場合は次の列車の予約を試みます。
取得した列車一覧に条件を満たすものがない場合はそれ以降の予約を停止します。
...
ただし、複数枚チケットを購入する団体旅行者はできるだけ同じ横の列に座りたがるため、割り当てられた席が必要以上に分割されいる場合は、チケットの金額は半額で販売することになります。例えば1列3席ある車両のスケジュールを6人分予約した際、2列分で席が確保できた場合は通常通り1区間6000円ですが、3列またはそれ以上に分割されてしまった場合は3000円になります。

座席の位置は `数字-アルファベット` で表現され、数字が列、アルファベットが席の位置を表します。例えば1列3席の幅の車両では、アルファベットは `A` から `C` まで存在し、 `3-A` は3列目のA席を表します。

例えば1列3席幅の車両に6人で予約する場合の金額計算は以下のようになります。
2列で6席が確保できた場合(1区間6000円で販売): `3-A`, `3-B`, `3-C`, `5-A`, `5-B`, `5-C`
3列に分割されてしまった場合(1区間3000円で販売): `3-B`, `3-C`, `4-A`, `4-B`, `4-C`, `5-A`
6列に分割されてしまった場合(1区間3000円で販売): `3-A`, `4-A`, `5-A`, `6-A`, `7-B`, `8-C`

売上を最大化するためには、できるだけグループ全員が同じ列に座れるように座席の配置アルゴリズムを工夫する必要があります。
...
鉄道会社のスタッフは最短で40分毎(現実世界で4秒)にダッシュボードで、その時点のチケットの購入枚数と確定売上の合計を確認します。好調であることが確認できれば、新たな車両が購入され、その車両とその車両に紐づく複数のスケジュールが登録されます。すべての新しい車両は既存の車両モデルのいずれかに紐づき、新たな車両モデルの車両が追加されることはありません。

ただし、アプリケーションが遅く、車両の登録等一連の処理に時間がかかってしまうと、鉄道会社のスタッフが次の行動を起こすまでに時間がかかってしまい、売上の最大化が阻害されてしまいます。スタッフの一連の行動に40分以上かかってしまった場合、次の売上チェックはその直後に行われます。
